...
Waiting is a model-chosen delay of that same continuation, not a state. `wait_goal` takes `delaySeconds` (at most one hour) and a required `waitingFor`, emits `goal_waiting`, and records the wake in its own `pi-goal-wake-v1` entry, so the Goal state shape is untouched and the Goal stays `active`. At the next idle boundary Goal holds the ordinary `Continue the Goal.` until the wake. Any run that starts first ends the wait, and with no wait pending continuation is immediate. A restart re-arms a still-future wake from the branch and continues at once when it has elapsed; leaving the active state ends the wait.
