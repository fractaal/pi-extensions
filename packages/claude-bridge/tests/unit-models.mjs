...
import { FABLE_FALLBACK_MODEL_ID, FABLE_MODEL_ID, MODEL_IDS_IN_ORDER, OPUS_5_5_MODEL_ID, OPUS_5_MODEL_ID, SONNET_5_MODEL_ID, buildModels, fallbackModelForPrimaryModel, resolveModelId } from "../src/models.js";
...
describe("MODELS projection", () => {
	...
	it("lists Fable 5 before Opus 5.5 and older Opus models", () => {
		const models = buildModels(MODEL_IDS_IN_ORDER.map(mockPiAiModel));
	...
	it("fills bridge-owned future IDs missing from pi-ai and drops unknown missing IDs", () => {
		...
		assert.deepEqual(models.map((m) => m.id), ["claude-fable-5", "claude-opus-5-5", "claude-opus-5", "claude-opus-4-8", "claude-sonnet-5", "claude-haiku-4-5"]);
