...
describe("loadConfig", () => {
	it("ignores project settings until project trust is recorded", () => withTempDirs(({ user, project }) => {
		writeFileSync(join(user, "settings.json"), JSON.stringify({
			vstack: { extensionManager: { config: { "@fractaal/pi-claude-bridge": { allowExtraUsage: false } } } },
		}));
		writeFileSync(join(project, ".pi", "settings.json"), JSON.stringify({
			vstack: { extensionManager: { config: { "@fractaal/pi-claude-bridge": { allowExtraUsage: true } } } },
		}));
	...
	it("maps the generic Pi system-prompt mode", () => withTempDirs(({ user, project }) => {
		writeFileSync(join(user, "settings.json"), JSON.stringify({
			vstack: { extensionManager: { config: { "@fractaal/pi-claude-bridge": { systemPromptMode: "claude-code" } } } },
		}));
		writeFileSync(join(project, ".pi", "settings.json"), JSON.stringify({
			vstack: { extensionManager: { config: { "@fractaal/pi-claude-bridge": { systemPromptMode: "pi" } } } },
		}));
		recordProjectTrust({ cwd: project, isProjectTrusted: () => true });

		const config = loadConfig(project);
		assert.equal(config.provider?.systemPromptMode, "pi");
	}));

	it("ignores invalid system-prompt modes", () => withTempDirs(({ user, project }) => {
		writeFileSync(join(user, "claude-bridge.json"), JSON.stringify({ provider: { systemPromptMode: "host-specific" } }));
		const config = loadConfig(project);
		assert.equal(config.provider?.systemPromptMode, undefined);
	}));
	...
	it("maps extension-manager effort overrides into provider config", () => withTempDirs(({ user, project }) => {
		writeFileSync(join(user, "settings.json"), JSON.stringify({
			vstack: { extensionManager: { config: { "@fractaal/pi-claude-bridge": {
				fastMode: false,
				forceEffort: "high",
