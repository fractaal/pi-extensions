...
export const PACKAGE_ID = "@fractaal/pi-claude-bridge";

export type BridgeEffortLevel = "low" | "medium" | "high" | "xhigh" | "max";
export type SystemPromptMode = "claude-code" | "pi";
...
function normalizeProviderConfig(provider: Config["provider"] | undefined): Config["provider"] {
	...
	if (raw.systemPromptMode === "claude-code" || raw.systemPromptMode === "pi") out.systemPromptMode = raw.systemPromptMode;
	else delete out.systemPromptMode;
...
function managerToConfig(raw: SettingsRecord): Partial<Config> {
	...
	if (raw.systemPromptMode === "claude-code" || raw.systemPromptMode === "pi") provider.systemPromptMode = raw.systemPromptMode;
